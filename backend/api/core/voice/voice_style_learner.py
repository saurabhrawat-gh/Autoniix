...
async def extract_voice_features(
    content_id: str, channel_id: str, audio_metrics: dict, emotion_data: list[dict], validation: dict
) -> dict:
    ...
    try:
        ...
        """,
            content_id,
            channel_id,
            features["avg_stability"],
            features["avg_similarity_boost"],
            features["avg_style"],
            features["avg_speed"],
...
async def predict_optimal_params(channel_id: str, niche: str) -> dict | None:
    ...
    try:
        ...
        logger.info(
            "voice_learner.predicted_optimal", channel_id=channel_id, params=optimal, model_auc=metrics.get("auc")
        )
...
async def ingest_voice_feedback(content_id: str, channel_id: str, retention_data: dict) -> bool:
...
async def train_voice_model(niche: str) -> dict:
    ...
    try:
        ...
        metrics = {
            ...
            "feature_importance": dict(zip(feature_cols, [round(float(fi), 4) for fi in model.feature_importances_])),
        }
