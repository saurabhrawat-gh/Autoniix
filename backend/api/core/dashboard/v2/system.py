...
async def get_config(_: Principal = Depends(principal_dep)):
    ...
    return {
        "status": "ok",
        "data": [{"key": r["config_key"], "value": r["config_value"], "description": r["description"]} for r in rows],
    }
...
async def emergency_stop(
    ...
    await audit(actor=actor, action="system.emergency_stop", target_type="system", target_id="global", request=request)
    return result
...
async def emergency_resume(
    ...
    await audit(
        actor=actor, action="system.emergency_resume", target_type="system", target_id="global", request=request
    )
    return result
...
async def get_environment(
    ...
    pool = await get_pool()
    row = await pool.fetchrow("SELECT config_value FROM system_config WHERE config_key = 'environment_mode'")
...
async def clean_slate(
    ...
    await audit(actor=actor, action="system.clean_slate", target_type="system", target_id="global", request=request)
    return result
