...
export default function ProvidersLayout({ children }: { children: ReactNode }) {
  ...
  const handleWipe = async () => {
    const phrase = await promptDialog({
      ...
      description: "This will DELETE every credential and chain in the database. " + "This cannot be undone.",
  ...
  const grouped: Record<string, any[]> = cats.reduce(
    (acc, c) => {
  ...
  const connectedCount = (catName: string) => creds.filter((cr) => cr.category === catName).length;
    ...
    <div className="flex flex-1 min-h-0 overflow-hidden">
      ...
      >
        ...
        <div className="h-12 px-3 flex items-center gap-2 border-b border-border shrink-0">
          ...
          <span className="text-sm font-semibold text-content-primary">Providers</span>
        </div>
                ...
                {creds.length > 0 && (
                  <span
                    className={cn(
                      ...
                      !activeCategory ? "bg-accent/20 text-accent" : "bg-surface-2 text-content-tertiary"
                    )}
                  >
                    {creds.length}
                  </span>
                                ...
                                {count > 0 && (
                                  <span
                                    className={cn(
                                      ...
                                      isActive ? "bg-accent/20 text-accent" : "bg-surface-2 text-content-tertiary"
                                    )}
                                  >
                                    {count}
                                  </span>
        ...
        <div className="shrink-0 p-3 border-t border-border space-y-2">
          ...
          {role === "owner" && !loading && (
            ...
            >
              {wiping ? <Loader2 size={11} className="animate-spin" /> : <Trash2 size={11} />}
        ...
        {activeCategory && (
          <nav className="px-4 sm:px-6 pt-4 pb-0 shrink-0" aria-label="Breadcrumb">
            <ol className="flex items-center gap-1.5 text-xs text-content-tertiary">
              <li>
                <Link href="/dashboard/providers" className="hover:text-content-primary transition-colors">
                  Providers
                </Link>
              </li>
              <li aria-hidden="true">
