...
const COLUMNS: { key: string; label: string; matches: (s: string) => boolean }[] = [
  ...
  { key: "review", label: "Awaiting review", matches: (s) => s === "pending_review" },
  {
    key: "published",
    label: "Published",
    matches: (s) => ["published", "delivered", "test_delivered", "completed"].includes(s),
  },
...
export default function KanbanPage() {
  ...
  const refresh = () => {
    setLoading(true);
    contentApi
      .list({ channel_id: channelId || undefined, group: "month", limit: 200 })
  ...
  return (
    <main className="flex-1 px-4 sm:px-6 py-6 max-w-[1600px] mx-auto w-full">
      <div className="flex items-center gap-3 mb-5">
        ...
        <Button type="button" variant="outline" size="icon-sm" onClick={refresh} title="Refresh" aria-label="Refresh">
        ...
        {byCol.map((col) => (
          <div key={col.key} className="rounded-xl border border-border bg-surface-1 flex flex-col min-h-[60vh]">
            <div className="px-3 py-2 border-b border-border flex items-center gap-2">
              ...
              <span className="ml-auto text-[10px] px-1.5 py-0.5 rounded bg-surface-2 text-content-tertiary">
                {col.items.length}
              </span>
            </div>
