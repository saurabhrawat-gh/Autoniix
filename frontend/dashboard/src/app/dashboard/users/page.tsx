...
export default function Users() {
  ...
  useEffect(() => {
    ...
    authApi
      .me()
      .then((r: any) => setMyId(r?.data?.user_id ?? null))
      .catch(() => {});
  }, []);
      ...
      <div className="space-y-5">
        <div>
          ...
          <p className="text-sm opacity-70 mt-1">
            Platform-wide accounts. <strong>Global role</strong> controls platform admin access. Each user&apos;s{" "}
            <strong>workspace role</strong> (owner / member / viewer) is shown per workspace below.
          </p>
        </div>
          ...
          <DialogContent size="sm">
            ...
            <DialogBody>
              <p className="text-sm text-content-secondary">
                <strong className="text-content-primary">You will immediately lose platform admin access.</strong> This
                action takes effect on your next page load.
              </p>
              <DialogFooter>
            ...
            return (
              <div key={u.id} className="divide-y divide-border/50">
                <div className={`p-3 flex items-center gap-3 ${u.disabled ? "opacity-50" : ""}`}>
                  ...
                  {/* Avatar */}
                  <div
                    className={`size-8 rounded-full text-white text-xs flex items-center justify-center shrink-0
                    ${
                      isSuperadmin
                  ...
                  <span
                    className={`text-[11px] px-2 py-0.5 rounded-full font-semibold shrink-0 ${GLOBAL_BADGE[u.global_role] ?? GLOBAL_BADGE.user}`}
                  >
                  ...
                  <div className="bg-surface-1/50">
                    {workspaces.length === 0 ? (
                      ...
                      workspaces.map((ws: any) => (
                        <div
                          key={ws.workspace_id}
                          className="pl-12 pr-4 py-2 flex items-center gap-3 border-b border-border/30 last:border-0"
                        >
                          ...
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${WS_ROLE_BADGE[ws.workspace_role] ?? WS_ROLE_BADGE.viewer}`}
                          >
                            {ws.workspace_role}
                          </span>
                        </div>
        ...
        <p className="text-xs text-content-tertiary">
          <strong>Superadmin</strong>: single platform-wide seat · full admin access · use Transfer to hand off
          ownership.
          <br />
