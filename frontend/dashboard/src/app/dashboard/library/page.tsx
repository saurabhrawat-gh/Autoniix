      ...
      <div className="flex gap-3 flex-1 min-h-0">
        ...
        <div className="w-[180px] shrink-0 flex flex-col gap-3 overflow-y-auto">
          ...
          <div className="rounded-xl border border-border bg-surface-0 overflow-hidden">
            ...
            >
              <span className="flex items-center gap-1.5">
                <ShieldCheck size={12} /> Brand Kits
              </span>
          ...
          {loading ? (
            ...
            >
              {Array.from({ length: 12 }).map((_, i) => (
                <div
                  key={i}
                  className={cn("rounded-lg bg-surface-2 animate-pulse", viewMode === "grid" ? "aspect-video" : "h-11")}
                />
              ))}
            </div>
          ) : items.length === 0 ? (
            ...
            >
              <UploadCloud
                size={28}
                className={cn("mb-3", dragOver ? "text-accent" : "text-content-tertiary opacity-40")}
              />
              <div className="text-sm font-medium text-content-primary">
                No assets in {scope} · {kind === "all" ? "all kinds" : kind}
              </div>
              ...
              {items.map((item: any, i: number) => (
                ...
                >
                  <div className="aspect-video bg-surface-2 flex items-center justify-center relative">
                    {item.thumbnail_key ? (
                      <img
                        src={`/api/v2/storage/thumb?key=${encodeURIComponent(item.thumbnail_key)}`}
                        alt=""
                    ...
                    <div className="absolute top-1 left-1">
                      <span className="text-[9px] px-1 py-0.5 rounded bg-surface-0/80 backdrop-blur-sm text-content-secondary">
                        {item.kind}
                      </span>
                    </div>
                  </div>
              ...
              {items.map((item: any, i: number) => (
                ...
                >
                  ...
                  {(item.tags || []).slice(0, 2).map((t: string) => (
                    <span
                      key={t}
                      className="text-[9px] px-1.5 py-0.5 rounded-full bg-surface-2 text-content-tertiary shrink-0"
                    >
                      {t}
                    </span>
                  ))}
        ...
        <div className="w-[240px] shrink-0 flex flex-col gap-3 overflow-y-auto">
          ...
          >
            {uploading ? (
              ...
              <UploadCloud
                size={20}
                className={cn("mx-auto mb-1.5", dragOver ? "text-accent" : "text-content-tertiary")}
              />
            )}
            ...
            <div className="text-[10px] text-content-tertiary">
              Scope: <span className="font-medium">{scope}</span> · Kind:{" "}
              <span className="font-medium">{kind === "all" ? "image" : kind}</span>
            </div>
              ...
              <div className="p-3 space-y-3">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {(detailAsset || selectedItem).tags.map((t: string) => (
                        <span
                          key={t}
                          className="text-[9px] px-1.5 py-0.5 rounded-full bg-surface-2 border border-border text-content-secondary"
                        >
                          {t}
                        </span>
                      ))}
                ...
                {detailAsset?.ai_tags && Object.keys(detailAsset.ai_tags).length > 0 && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {Object.entries(detailAsset.ai_tags)
                        .slice(0, 8)
                        .map(([t, c]: [string, any]) => (
                          <span
                            key={t}
                            className="text-[9px] px-1.5 py-0.5 rounded-full bg-accent/10 border border-accent/20 text-accent"
                          >
                ...
                {detailAsset?.versions?.length > 0 && (
                  <div>
                    <div className="text-[9px] uppercase tracking-wider text-content-tertiary mb-1">
                      Versions ({detailAsset.versions.length})
                    </div>
