...
export default function ProgressPage() {
  ...
  useEffect(() => {
    ...
    const isTerminal =
      expandedJob &&
      ["failed", "stopped", "superseded", "delivered", "test_delivered", "rejected"].includes(expandedJob.status);
    if (isTerminal)
      return () => {
        cancelled = true;
      };
              ...
              {Object.entries(grouped).map(([channelId, channelJobs]) => (
                <div key={channelId}>
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    <Link
                      href={`/dashboard/channels/${channelId}`}
                              ...
                              <div className="flex items-center gap-3 min-w-0 flex-1">
                                ...
                                <div className="min-w-0 flex-1">
                                  <div className="flex items-center gap-2 flex-wrap">
                                    <Link
                                      href={`/dashboard/jobs/${job.content_id}`}
                                    ...
                                    {isFailed && (
                                      <span className="badge bg-status-error/10 text-status-error text-[10px] shrink-0">
                                        Failed
                                      </span>
                                    )}
                                    {isStopped && (
                                      <span className="badge bg-status-warning/10 text-status-warning text-[10px] shrink-0">
                                        Stopped
                                      </span>
                                    )}
                                    {isPaused && !isFailed && !isStopped && (
                                      <span className="badge bg-status-warning/10 text-status-warning text-[10px] shrink-0">
                                        Paused
                                      </span>
                                    )}
                                  ...
                                  <div className="flex items-center gap-1.5">
                                    {job.checkpoint && (
                                      ...
                                      >
                                        {isRestarting
                                          ? "Restarting..."
                                          : `Restart from ${PHASE_LABELS[job.checkpoint] || job.checkpoint}`}
                                      </Button>
                                    )}
                                    <Button
                                      type="button"
                                  ...
                                  {timeline.length === 0 && !tlLoaded ? (
                                    ...
                                    <div className="space-y-0">
                                      {PHASE_ORDER.map((phase) => {
                                        ...
                                        const cost = phaseEvents.reduce(
                                          (sum: number, ev: any) => sum + (ev.cost_usd || 0),
                                          0
                                        );
                                          ...
                                          return (
                                            <div key={phase} className="flex items-start gap-3 py-2">
                                              ...
                                              <div className="flex-1 min-w-0 pt-0.5">
                                                <div className="text-xs text-content-tertiary/50">
                ...
                <DialogBody>
                  <p className="text-sm text-content-secondary leading-relaxed">
                    ...
                    {checkpointLabel ? (
                      <>
                        {" "}
                        If you only want to redo the later phases, open the timeline and hit{" "}
                        <span className="text-status-success font-medium">Restart from {checkpointLabel}</span> instead.
                      </>
                    ) : (
                      <>
                        {" "}
                        If you only want to redo the later phases, open the timeline and hit{" "}
                        <span className="text-status-success font-medium">Restart from checkpoint</span> instead.
                      </>
                    )}
                  </p>
                  <DialogFooter>
                    <Button variant="ghost" size="sm" onClick={() => setRetryConfirm(null)} disabled={running}>
                      Cancel
                    </Button>
                    <Button
                      size="sm"
                      onClick={async () => {
                        await handleRetry(retryConfirm);
