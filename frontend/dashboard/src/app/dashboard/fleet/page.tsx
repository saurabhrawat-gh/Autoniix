  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <main className="max-w-5xl mx-auto w-full px-6 py-8 space-y-6">
        ...
        {error && <div className="bg-status-error/10 text-status-error text-sm rounded-lg p-3">{error}</div>}
        ...
        {data && !loading && (
          <>
              ...
              >
                <div className="flex items-start justify-between gap-6">
                  <div className="flex-1">
                    <div className="text-xs uppercase tracking-wider text-content-tertiary mb-1">System health</div>
            ...
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-content-primary mb-1">
                ...
                <span className="text-content-tertiary">
                  ({data.services.ok_count}/{data.services.total} healthy)
                </span>
              </h2>
              <p className="text-xs text-content-tertiary mb-4">
                Each probe has a 3-second timeout. p99 latency is the leading indicator that a service is about to fall
                over.
              </p>
                ...
                {data.services.probes.map((p) => (
                  <div
                    ...
                    className={cn(
                      ...
                      p.ok ? "border-status-success/20 bg-status-success/5" : "border-status-error/30 bg-status-error/5"
                    )}
                  >
                    <div className="flex items-center justify-between text-xs">
                      <span className={cn("font-medium", p.ok ? "text-content-primary" : "text-status-error")}>
                        {p.name}
                      </span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <section className="card p-5">
                ...
                <Stat
                  ...
                  accent={
                    data.db_pool.pressure !== null && data.db_pool.pressure > 0.85
                      ? "warn"
                      : data.db_pool.pressure !== null && data.db_pool.pressure > 0.5
                        ? undefined
                        : "ok"
                  }
                />
              ...
              <section className="card p-5">
                ...
                {data.remotion.ok ? (
                  <>
                    ...
                    <Stat
                      label="Waiting"
                      value={String(data.remotion.waiting ?? 0)}
              ...
              <section className="card p-5">
                ...
                <Stat label="Quality-gate blocks" value={data.pressure_24h.quality_gate_blocks?.toString() ?? "—"} />
                <Stat
                  label="Failed videos"
            ...
            {/* Niche pulse (Phase 8) ─────────── */}
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-content-primary mb-1">Niche pulse</h2>
              <p className="text-xs text-content-tertiary mb-4">
                External saturation signal: recent competitor videos with title embeddings, used by the opportunity
                scorer to down-weight crowded topics. Refreshed weekly per niche.
              </p>
              {!data.niche_pulse?.ok ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Stat label="Niches with data" value={String(data.niche_pulse.niches_with_data ?? 0)} />
                  <Stat
            ...
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-content-primary mb-1">Diversity floor</h2>
              <p className="text-xs text-content-tertiary mb-4">
                Anti-mode-collapse for the bandits. The floor monitors per-channel arm-pick entropy and forces
                exploration when bandits start repeating themselves. Force rate of 5-15% is healthy; sustained &gt;25%
                suggests the threshold is too aggressive.
              </p>
              {!data.diversity_floor?.ok ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Stat label="Picks (7d)" value={String(data.diversity_floor.picks_7d ?? 0)} />
                  <Stat label="Forced (7d)" value={String(data.diversity_floor.forced_7d ?? 0)} />
                  <Stat
                    label="Force rate"
                    value={
                      data.diversity_floor.force_rate !== null && data.diversity_floor.force_rate !== undefined
                        ? `${Math.round(data.diversity_floor.force_rate * 100)}%`
                        : "—"
                    }
                    accent={
                      (data.diversity_floor.force_rate ?? 0) > 0.25
                        ? "warn"
                        : (data.diversity_floor.force_rate ?? 0) > 0
                          ? "ok"
                          : undefined
                    }
                  />
            ...
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-content-primary mb-1">Prediction calibration</h2>
              <p className="text-xs text-content-tertiary mb-4">
                Brier score and Expected Calibration Error over the last 30 days of scored topic-success predictions.
                Brier &lt; 0.20 is healthy; ECE &lt; 0.10 means the model&apos;s confidence is well-calibrated.{" "}
                <strong>Weighted fraction</strong> is the share of the latest training run&apos;s rows that received a
                confidence-weight boost from past misses — non-zero means the correction loop is steering retraining.
              </p>
              {!data.calibration?.ok ? (
                <div className="text-xs text-content-tertiary italic">
                ...
                <div className="grid grid-cols-4 gap-3">
                  <Stat label="Scored (30d)" value={String(data.calibration.n ?? 0)} />
                  <Stat
                    label="Brier"
                    value={
                      data.calibration.brier !== null && data.calibration.brier !== undefined
                        ? data.calibration.brier.toFixed(3)
                        : "—"
                    }
                    accent={
                      (data.calibration.brier ?? 1) < 0.2
                        ? "ok"
                        : (data.calibration.brier ?? 0) > 0.3
                          ? "warn"
                          : undefined
                    }
                  />
                  <Stat
                    label="ECE"
                    value={
                      data.calibration.ece !== null && data.calibration.ece !== undefined
                        ? data.calibration.ece.toFixed(3)
                        : "—"
                    }
                    accent={
                      (data.calibration.ece ?? 1) < 0.1 ? "ok" : (data.calibration.ece ?? 0) > 0.2 ? "warn" : undefined
                    }
                  />
                  <Stat
                    label="Weighted frac"
                    value={
                      data.calibration.weighted_fraction !== null && data.calibration.weighted_fraction !== undefined
            ...
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-content-primary mb-1">Retention-curve coverage</h2>
              <p className="text-xs text-content-tertiary mb-4">
                Of delivered videos in the curve-stable window (7-30 days old), how many have an audience-retention
                curve fetched. The Phase 7 calibrator uses these curves directly for hook and pacing dimensions; videos
                without curves fall back to tier-based labels.
              </p>
              {!data.retention_coverage?.ok ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Stat label="Eligible videos" value={String(data.retention_coverage.eligible ?? 0)} />
                  <Stat
                    label="With curve"
                  ...
                  <Stat
                    label="Coverage"
                    value={
                      data.retention_coverage.coverage !== null && data.retention_coverage.coverage !== undefined
                        ? `${Math.round(data.retention_coverage.coverage * 100)}%`
                        : "—"
                    }
                    accent={
                      (data.retention_coverage.coverage ?? 0) >= 0.7
                        ? "ok"
                        : (data.retention_coverage.coverage ?? 1) < 0.3
                          ? "warn"
                          : undefined
                    }
                  />
            ...
            {/* Gate calibration ─────────── */}
            <section className="card p-5">
              <h2 className="text-sm font-semibold text-content-primary mb-1">Quality-gate calibration</h2>
              <p className="text-xs text-content-tertiary mb-4">
                Per-niche thresholds learned from real outcomes (Phase 7). Auto rows mean the gate is using a tuned
                floor; default rows mean the niche hasn't accumulated enough data yet.
              </p>
              {!data.gate_calibration?.ok ? (
                ...
                <div className="grid grid-cols-3 gap-3">
                  <Stat label="Niches calibrated" value={String(data.gate_calibration.niches_calibrated ?? 0)} />
                  <Stat label="Tuned dimensions" value={String(data.gate_calibration.dims_auto ?? 0)} accent="ok" />
                  <Stat label="Awaiting data" value={String(data.gate_calibration.dims_default ?? 0)} />
                </div>
              )}
            ...
            <section className="card p-5">
              ...
              <div className="grid grid-cols-3 gap-3">
                <Stat
                  label="Prod max activities"
                  value={String(data.scale_config.temporal_production_max_activities)}
                />
                <Stat
                  label="Scheduler max activities"
                  value={String(data.scale_config.temporal_scheduler_max_activities)}
                />
...
function Stat({ label, value, accent }: { label: string; value: string; accent?: "ok" | "warn" }) {
  const color =
    accent === "warn" ? "text-status-warning" : accent === "ok" ? "text-status-success" : "text-content-primary";
  return (
