...
export default function ReviewDetail() {
  ...
  const [decisionModal, setDecisionModal] = useState<{ open: boolean; decision: string }>({
    open: false,
    decision: "",
  });
    ...
    <main className="flex-1 px-4 sm:px-6 py-6 max-w-[1500px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr_300px] gap-4">
        ...
        <aside className="rounded-xl border border-border bg-surface-0 p-3 flex flex-col">
          <div className="px-2.5 mb-2 text-[10px] uppercase tracking-widest font-semibold text-content-tertiary">
            Artifacts
          </div>
            ...
            {ARTIFACTS.map((a) => (
              <li key={a.key}>
                <Button
                  ...
                  className={cn(
                    "w-full justify-start gap-3 px-2.5 py-2 h-auto text-sm font-medium",
                    tab === a.key
          ...
          <div className="mt-auto pt-3 border-t border-border text-[11px] text-content-tertiary">
            {data.script_versions.length} script version{data.script_versions.length !== 1 ? "s" : ""} ·{" "}
            {data.thumbnail_versions.length} thumbnail{data.thumbnail_versions.length !== 1 ? "s" : ""}
          </div>
        </aside>
        ...
        <div className="rounded-xl border border-border bg-surface-0 p-5 min-h-[640px] flex flex-col">
          ...
          <div className="flex items-start gap-3 mb-5 pb-4 border-b border-border">
            <div className="min-w-0 flex-1">
              ...
              <h1 className="font-semibold text-lg text-content-primary truncate">
            ...
            {session?.state && (
              <span
                className={cn(
                  "text-[10px] uppercase px-2.5 py-1 rounded-md font-medium shrink-0",
                  session.state === "approved"
                    ? "bg-status-success/15 text-status-success"
                    : session.state === "rejected"
                      ? "bg-status-error/15 text-status-error"
                      : session.state === "needs_edits"
          ...
          <div className="flex-1 min-h-0">
            {tab === "script" && (
              <ScriptPane
                ...
                onVariantChange={setScriptVariant}
                onChange={refresh}
              />
            )}
          ...
          <div className="mt-6 pt-4 border-t border-border flex items-center justify-end gap-2">
            ...
            <DecisionButton
              onClick={() => openDecision("needs_edits")}
              icon={RotateCw}
              label="Request edits"
              tone="amber"
            />
        ...
        <aside className="rounded-xl border border-border bg-surface-0 p-4 flex flex-col h-full max-h-[calc(100vh-160px)]">
          <div className="text-[11px] uppercase tracking-wider text-content-tertiary font-semibold mb-3">
            Comments ({data.comments.length})
          </div>
          <div className="flex-1 overflow-y-auto scrollbar-hide space-y-3 mb-3 pr-1">
            {data.comments.length === 0 && (
              <div className="text-xs text-content-tertiary py-4 text-center">No comments yet.</div>
            )}
            {data.comments.map((c: any) => (
              <div key={c.id} className="text-xs bg-surface-1 rounded p-3 border border-border/50">
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-content-tertiary text-[10px] ml-auto">
      ...
      <Dialog
        ...
        onOpenChange={(o) => {
          if (!o) setDecisionModal({ open: false, decision: "" });
        }}
      >
        <DialogContent size="sm">
            ...
            <div>
              <DialogTitle>
                {decisionModal.decision === "approved"
                  ? "Approve content?"
                  : decisionModal.decision === "needs_edits"
                    ? "Request edits"
                    : "Reject content"}
              </DialogTitle>
              <DialogDescription>
                {decisionModal.decision === "approved"
                  ? "This content will be marked as approved and queued for delivery."
                  : decisionModal.decision === "needs_edits"
                    ? "Add a note describing what needs to be changed."
                    : "This content will be marked as rejected and archived."}
              </DialogDescription>
            </div>
          ...
          <DialogBody>
            ...
            />
            <DialogFooter>
              <Button variant="ghost" size="sm" onClick={() => setDecisionModal({ open: false, decision: "" })}>
                Cancel
              </Button>
              <Button
                size="sm"
...
function DecisionButton({
  ...
  const toneMap: Record<string, string> = {
    accent: primary ? "bg-accent text-white hover:bg-accent-hover" : "bg-accent/10 text-accent hover:bg-accent/15",
  ...
  return (
    <Button type="button" variant="ghost" onClick={onClick} leftIcon={<Icon size={14} />} className={cn(toneMap[tone])}>
      {label}
    </Button>
  );
}
    ...
    <div className="space-y-3 h-full flex flex-col">
      ...
      {versions.length > 1 && (
        <div className="mt-2 pt-2 border-t border-border">
          <div className="text-[11px] font-semibold uppercase tracking-wider text-content-tertiary mb-2">
            Version history
          </div>
          <ul className="space-y-1">
            {versions.map((v: any) => (
              <li
                key={v.id}
                className="text-xs flex items-center gap-2 px-2 py-1.5 rounded hover:bg-surface-1 transition-colors"
              >
    ...
    <div className="space-y-3">
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {(data.thumbnail_versions || []).map((t: any) => (
          <div
            key={t.id}
            className="rounded border border-border p-2 hover:border-accent/30 transition-colors cursor-pointer"
          >
        ...
        {data.thumbnail_versions?.length === 0 && (
          <div className="text-xs text-content-tertiary col-span-full">No thumbnail variants yet.</div>
        )}
      </div>
      <div className="flex gap-2 pt-2 border-t border-border">
        <Input
          ...
          placeholder="Optional prompt nudge…"
          className="flex-1"
        />
...
function TitlePane({ data, videoId, onChange }: any) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <FieldLabel className="text-[11px] font-semibold uppercase tracking-wider text-content-tertiary mb-2 block">
          Title
        </FieldLabel>
