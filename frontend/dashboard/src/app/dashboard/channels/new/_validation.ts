...
export function validateAutomation(state: any): StepValidation {
  ...
  if (sw + lw === 0) {
    errors.videos_per_week_short = errors.videos_per_week_short
      || 'You must publish at least 1 video per week (short or long).';
  }
...
const STEP_KEYS = [
  'basics', 'strategy', 'pillars', 'voice', 'visual',
  'references', 'automation', 'review',
] as const;
...
export function computeAllValidity(state: any): Record<StepKey, boolean> {
  ...
  STEP_KEYS.forEach(k => { out[k] = validateStep(k, state).valid; });
  return out;
}
