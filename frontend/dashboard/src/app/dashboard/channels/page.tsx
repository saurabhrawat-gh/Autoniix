...
export default function ChannelsPage() {
  ...
  async function triggerChannel(id: string, contentMode: string) {
    ...
    try {
      await channelsApi.trigger(id, { content_mode: contentMode });
      queryClient.invalidateQueries({ queryKey: qk.channels.all });
    } catch (e: any) {
  ...
  async function stopJob(channelId: string, contentId: string) {
    ...
    setBusyJobs((prev) => new Set(prev).add(contentId));
    try {
      await channelsApi.stopJob(channelId, contentId);
      queryClient.invalidateQueries({ queryKey: qk.channels.all });
    } catch (e: any) {
  ...
  function getModeJob(ch: any, mode: string) {
  ...
  function getWeeklyLabel(ch: any): string | null {
    ...
    return getModes(ch)
      .map((m: string) => {
        ...
        return u ? `${u.used}/${u.limit}${m === "short" ? "S" : "L"}` : null;
      })
      .filter(Boolean)
      .join(" · ");
  }
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <Button
                variant="outline"
                size="icon-sm"
                onClick={() => queryClient.invalidateQueries({ queryKey: qk.channels.all })}
                aria-label="Refresh"
              >
                <RotateCw size={13} />
              </Button>
        ...
        <div className="flex-1 min-h-0 max-w-[1400px] w-full mx-auto px-6 pb-4 overflow-y-auto">
          {displayChannels.length === 0 ? (
            <div className="rounded-xl border border-border bg-surface-0 p-8">
              {debouncedSearch ? (
                ...
                <EmptyState
                  ...
                  body="Create your first channel to start producing videos automatically."
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <Link
                          href={`/dashboard/channels/${ch.channel_id}`}
                        ...
                        <div className="flex items-center gap-1.5 mt-1 flex-wrap">
                          ...
                          {ch.auto_upload && (
                            <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/10 text-accent">Auto</span>
                          )}
                        </div>
                      </div>
                        ...
                        <>
                          {modes.map((m: string) => {
                            ...
                            if (mState === "pending_review" && mJob)
                              return (
                                <Link
                                  key={m}
                                  href={`/dashboard/jobs/${mJob.content_id}`}
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-44">
                              ...
                              <DropdownMenuItem onClick={() => exportChannel(ch.channel_id)}>
                                Export JSON
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem
                                onClick={() => setConfirmArchive(ch.channel_id)}
                                className="text-status-warning focus:text-status-warning focus:bg-status-warning/5"
                              >
                                Archive
                              </DropdownMenuItem>
                            </DropdownMenuContent>
            ...
            <div
              className={cn(
                "rounded-xl border border-border bg-surface-0 overflow-hidden",
                systemStopped && "lockdown-frost"
              )}
            >
                ...
                {displayChannels.map((ch: any) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2.5 min-w-0 pr-3">
                        ...
                        <Link href={`/dashboard/channels/${ch.channel_id}`} className="min-w-0">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <span className="font-medium text-sm text-content-primary hover:text-accent truncate">
                              {ch.channel_name}
                            </span>
                            {ch.niche && (
                              <span className="text-[10px] px-1.5 py-0.5 rounded bg-surface-2 text-content-tertiary">
                            ...
                            {ch.auto_upload && (
                              <span className="text-[10px] px-1 py-0.5 rounded bg-accent/10 text-accent">Auto</span>
                            )}
                            {isArchived && (
                              <span className="text-[10px] px-1 py-0.5 rounded bg-surface-3 text-content-tertiary">
                                Archived
                              </span>
                            )}
                          </div>
                          <div className="text-[10px] text-content-tertiary mt-0.5 font-mono truncate">
                            {ch.channel_id}
                          </div>
                        </Link>
                      </div>
                      ...
                      {/* Delivered */}
                      <div className="text-center text-sm font-semibold text-content-primary tabular-nums">
                        {ch.stats?.delivered || 0}
                      </div>

                      {/* In progress */}
                      <div className="text-center text-sm text-content-tertiary tabular-nums">
                        {ch.stats?.in_progress || 0}
                      </div>
                          ...
                          <>
                            {(() => {
                              const hasRunning = modes.some((m: string) =>
                                ["running", "paused"].includes(getModeState(ch, m))
                              );
                              return (
                                        ...
                                        {mJob && (
                                          <>
                                            <Button
                                              ...
                                              size="sm"
                                              onClick={() =>
                                                togglePauseJob(ch.channel_id, mJob.content_id, mJob.is_paused)
                                              }
                                              disabled={!!isBusy}
                                              aria-label={mJob.is_paused ? "Resume" : "Pause"}
                                              className={cn(
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="w-44">
                                <DropdownMenuItem onClick={() => cloneChannel(ch.channel_id)}>
                                  Duplicate Channel
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => exportChannel(ch.channel_id)}>
                                  Export Config (JSON)
          ...
          <DialogContent size="sm">
            <DialogHeader>
              <div>
                ...
                <DialogDescription>
                  <span className="font-medium text-content-secondary">{confirmArchive}</span> will be hidden from the
                  active list. All configuration and history is preserved and can be restored.
                </DialogDescription>
              </div>
