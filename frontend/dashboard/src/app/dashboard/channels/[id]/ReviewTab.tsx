...
function gatesForProfile(profile: ReviewProfile): ReviewGates {
  ...
  if (profile === "full_control")
    return {
      research_data: true,
      brand_alignment_report: true,
      topic_title: true,
      story_script: true,
      script_voice_data: true,
      script_assets_data: true,
      script_direction_data: true,
      voice_track: true,
      scene_images: true,
      remotion_v3_json: true,
      metadata: true,
      thumbnail: true,
      final_video: true,
    };
...
export default function ReviewTab({ channelId }: Props) {
  ...
  if (loading) {
    return <div className="p-6 text-sm text-muted-foreground">Loading review config…</div>;
  }
  ...
  return (
    <div className="space-y-8 p-1">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            ...
            {activeGateCount > 0 && (
              <span className="ml-1 text-status-warning font-medium">
                {activeGateCount} gate{activeGateCount !== 1 ? "s" : ""} active
              </span>
            )}
          </p>
        </div>
      ...
      <section className="space-y-3">
        ...
        <p className="text-xs text-muted-foreground">{PROFILES.find((p) => p.value === profile)?.description}</p>
      </section>
        ...
        <div className="divide-y divide-border rounded-lg border border-border">
          {GATE_META.map(({ key, label, phase }) => (
            <div key={key} className="flex items-center justify-between px-4 py-3 gap-4">
              ...
              <Switch checked={gates[key]} onCheckedChange={() => toggleGate(key)} />
            </div>
          ))}
        </div>
      </section>
