...
export default function ChannelDetail() {
  ...
  const refresh = async () => {
    ...
    setError(null);
    try {
      const [r, me] = await Promise.all([channelsApi.get(id), authApi.me().catch(() => null)]);
      setData(r.data);
      setDraft({});
  ...
  const handleDelete = async () => {
    ...
    const ok = await confirmDialog({
      ...
      description:
        "Permanently removes this channel, its brand profile, topics, series, and memory. Published videos are not deleted — migrate or archive them first.",
    ...
    try {
      ...
      const msg =
        code === "has_videos"
          ? e?.detail?.message || "Cannot delete a channel that has published videos. Archive instead."
          : code === "channel_not_found"
            ? "This channel is not in your active workspace. Switch to the owning workspace and try again."
  ...
  const save = async () => {
    setSaving(true);
    try {
      await channelsApi.patch(id, draft);
      await refresh();
    } finally {
      setSaving(false);
    }
    ...
    return (
      <main className="flex-1 px-4 sm:px-6 py-6 max-w-[1400px] mx-auto w-full">
        <Link
          ...
          className="inline-flex items-center gap-1 text-xs text-content-tertiary hover:text-content-primary mb-4"
        >
        ...
        <div className="max-w-md mx-auto mt-12 rounded-md border border-status-error/30 bg-status-error/5 p-6 text-center">
          ...
          <p className="text-sm text-content-tertiary mb-4">
            {error || `Channel "${id}" was not found or the backend returned an error.`}
          </p>
          <div className="flex gap-2 justify-center">
            <Button size="sm" onClick={refresh} leftIcon={<RefreshCw size={12} />}>
              Retry
            </Button>
    ...
    <main className="flex-1 px-4 sm:px-6 py-6 max-w-[1400px] mx-auto w-full">
      <div className="max-w-5xl">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          {c.content_mode && (
            <div className="flex items-center gap-1">
              ...
              {(c.content_mode === "both" ||
                c.content_mode === "mixed" ||
                c.content_mode === "long" ||
                c.content_mode === "long_form") && <span className="chip-long">Long</span>}
            </div>
          )}
        </div>
        ...
        <div className="rounded-xl border border-border bg-surface-0 p-6 space-y-4">
          {tab === "Basics" && <BasicsTabContent get={get} set={set} />}
          {tab === "Strategy" && (
            <Grid>
              <div className="block">
              ...
              <Inp
                ...
                value={get("publish_cadence") || ""}
                onChange={(v: any) => set("publish_cadence", v)}
              />
              <Inp label="Tone" value={get("tone") || ""} onChange={(v: any) => set("tone", v)} />
              <Inp
                label="Brand personality"
                wide
                value={get("brand_personality") || ""}
                onChange={(v: any) => set("brand_personality", v)}
              />
              <div className="md:col-span-2 block">
                <FieldLabel className="text-xs uppercase tracking-wide opacity-70 mb-1 block">
                  Channel type tags
                </FieldLabel>
          ...
          {tab === "Voice" && (
            <Grid>
              <Inp
                ...
                value={get("narration_style") || ""}
                onChange={(v: any) => set("narration_style", v)}
              />
              ...
              <Inp
                ...
                value={get("elevenlabs_voice_id") || ""}
                onChange={(v: any) => set("elevenlabs_voice_id", v)}
              />
              <Inp
                label="Emotion intensity"
                type="number"
                value={get("emotion_intensity") ?? 0}
                onChange={(v: any) => set("emotion_intensity", Number(v))}
              />
              <Inp
                label="Voice stability"
                type="number"
                value={get("voice_stability") ?? 0}
                onChange={(v: any) => set("voice_stability", Number(v))}
              />
              <Inp
                label="Voice similarity"
                type="number"
                value={get("voice_similarity") ?? 0}
                onChange={(v: any) => set("voice_similarity", Number(v))}
              />
              <Inp
                label="Voice style"
                type="number"
                value={get("voice_style") ?? 0}
                onChange={(v: any) => set("voice_style", Number(v))}
              />
            </Grid>
          )}
          {tab === "Visual" && (
            <Grid>
              <Inp
                ...
                wide
                value={get("thumbnail_style") || ""}
                onChange={(v: any) => set("thumbnail_style", v)}
              />
              <Inp
                label="Typography"
                value={get("typography_preference") || ""}
                onChange={(v: any) => set("typography_preference", v)}
              />
              <Inp
                label="LUT preference"
                value={get("lut_preference") || ""}
                onChange={(v: any) => set("lut_preference", v)}
              />
              <Inp
                label="Transitions"
                value={get("transition_preference") || ""}
                onChange={(v: any) => set("transition_preference", v)}
              />
              <Inp
                label="Primary color"
                value={get("primary_color") || ""}
                onChange={(v: any) => set("primary_color", v)}
              />
              <Inp
                label="Secondary color"
                value={get("secondary_color") || ""}
                onChange={(v: any) => set("secondary_color", v)}
              />
              <Inp
                label="Meme intensity"
                type="number"
                value={get("meme_intensity") ?? 0}
                onChange={(v: any) => set("meme_intensity", Number(v))}
              />
            </Grid>
          )}
          {tab === "Pillars" && <PillarsTab data={data} channel_id={id} onChange={refresh} />}
          {tab === "References" && <ReferencesTab data={data} channel_id={id} onChange={refresh} />}
          {tab === "Providers" && <ProvidersTab channelId={id} />}
          {tab === "Finishing" && <FinishingTab channelId={id} />}
          {tab === "Review" && <ReviewTab channelId={id} />}
          {tab === "Automation" && (
            <Grid>
              <Inp
                label="Auto upload"
                type="checkbox"
                value={get("auto_upload") ? "true" : ""}
                onChange={(v: any) => set("auto_upload", !!v)}
              />
              <Inp
                label="Human review"
                value={get("human_review_required") || ""}
                onChange={(v: any) => set("human_review_required", v)}
              />
              <Inp
                label="Forced review ratio"
                type="number"
                value={get("human_review_ratio") ?? 0}
                onChange={(v: any) => set("human_review_ratio", Number(v))}
              />
              <Inp
                label="Review timeout (h)"
                type="number"
                value={get("review_timeout_hours") ?? 24}
                onChange={(v: any) => set("review_timeout_hours", Number(v))}
              />
              <Inp
                label="Daily API spend cap"
                type="number"
                value={get("max_daily_api_spend") ?? 0}
                onChange={(v: any) => set("max_daily_api_spend", Number(v))}
              />
              <Inp
                label="Videos / week (short)"
                type="number"
                value={get("videos_per_week_short") ?? 0}
                onChange={(v: any) => set("videos_per_week_short", Number(v))}
              />
              <Inp
                label="Videos / week (long)"
                type="number"
                value={get("videos_per_week_long") ?? 0}
                onChange={(v: any) => set("videos_per_week_long", Number(v))}
              />
              <Inp
                label="Short duration (s)"
                type="number"
                value={get("short_form_duration") ?? 60}
                onChange={(v: any) => set("short_form_duration", Number(v))}
              />
              <Inp
                label="Long duration (s)"
                type="number"
                value={get("long_form_duration") ?? 600}
                onChange={(v: any) => set("long_form_duration", Number(v))}
              />
            </Grid>
          )}
          {tab === "Memory" && (
            <div>
              ...
              {data.memory.length === 0 && (
                <div className="text-xs opacity-60">No memory yet. Reviews + outcomes will populate this.</div>
              )}
          ...
          <section className="mt-6 border border-status-error/30 rounded-xl overflow-hidden">
            ...
            <div className="px-5 py-4 space-y-3">
              <p className="text-sm text-content-secondary">
                <strong>Permanently delete this channel.</strong> This removes the channel along with its pillars,
                references, topic rules, memory entries, projects, series, and unlinks any provider credentials.
                Published videos are kept and detached. <strong>This action cannot be undone.</strong>
              </p>
              <Button
                variant="outline"
    ...
    <div>
      ...
      <div className="flex gap-2">
        ...
        <Button
          onClick={async () => {
            if (!draftName) return;
            await channelsApi.addPillar(channel_id, {
              name: draftName,
  ...
  return (
    <Grid>
      ...
      <div className="block">
        ...
        <LvSelect
          opts={ageGroups}
          value={get("target_age_group") || ""}
          onChange={(v) => set("target_age_group", v)}
      ...
      <Inp
        ...
        onChange={(v: any) => set("target_audience", v)}
      />
      <Inp
        label="Description"
        wide
