...
`dashboard/src/app/globals.css` under `:root` (light) and `.dark` (dark). Both
themes share the **same names** — switching `class="dark"` on `<html>` is the
only thing required to flip the entire UI.
