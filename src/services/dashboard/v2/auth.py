...
def _issue_jwt(
    ...
    ``role``        — workspace-scoped role (owner|member|viewer)
    ...
    return jwt.encode(
        {
            "sub": str(user["id"]),
            "email": user["email"],
            "role": ws_role or "viewer",   # workspace role
            "global_role": global_role,     # platform role
            "wid": workspace_id,
            "iat": int(time.time()),
...
async def register(request: Request, body: RegisterIn):
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            ...
            # #350: Public self-serve signup. The first registrant bootstraps
            # as the platform superadmin; all subsequent self-registered users
            # get the non-privileged 'user' global role. Workspace-level
            # ownership is granted via workspace_members.role='owner'.
...
async def delete_account(
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            await conn.execute(
                "UPDATE sessions SET revoked_at=NOW() WHERE user_id=$1 AND revoked_at IS NULL", p.user_id
            )
            await conn.execute("DELETE FROM workspace_members WHERE user_id=$1", p.user_id)
            await conn.execute(
                """UPDATE users SET
