...
async def set_flag(
    ...
    before = await pool.fetchrow(
        "SELECT enabled, payload FROM feature_flags WHERE key=$1", key
    )
    if not before:
    ...
    await audit(actor=actor, action="flag.update", target_type="feature_flag",
                target_id=key, before=dict(before), after={"enabled": enabled, "payload": payload},
                request=request)
    return {"status": "ok"}
