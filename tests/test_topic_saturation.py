...
def test_many_perfect_matches_saturate():
    ...
    rows = [_PulseRow(similarity=1.0, view_velocity=VELOCITY_REFERENCE, age_days=0.0) for _ in range(8)]
    res = compute_saturation_from_pulse(rows)
...
def test_below_cosine_threshold_treated_as_no_match():
    eps = 0.01
    rows = [
        _PulseRow(similarity=COSINE_THRESHOLD - eps, view_velocity=VELOCITY_REFERENCE, age_days=0.0),
    ] * 10
    res = compute_saturation_from_pulse(rows)
    assert res.n_matches == 0
    assert res.saturation == 0.0
