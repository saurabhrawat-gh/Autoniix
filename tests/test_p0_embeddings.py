...
async def test_embed_text_returns_vector_and_records_cost(mock_pool):
    ...
    with patch("llm.embeddings.settings.openai_api_key", "sk-test"), patch("llm.embeddings.httpx.AsyncClient", _Client):
...
async def test_embed_text_retries_on_5xx_then_succeeds(mock_pool):
    ...
    with (
        patch("llm.embeddings.settings.openai_api_key", "sk-test"),
...
async def test_embed_and_store_updates_target_row(mock_pool):
    ...
    update_calls = [c for c in mock_pool.execute.await_args_list if "UPDATE" in (c.args[0] if c.args else "")]
