...
export function signinLoadTest() {
  ...
  const resp = http.post(`${RUST_BASE}/api/v2/auth/signin`, JSON.stringify({ email, password }), params);
  check(resp, {
...
export function meLoadTest() {
  ...
  const signin = http.post(`${RUST_BASE}/api/v2/auth/signin`, JSON.stringify({ email, password }), params);
  let token = null;
  try {
