...
async def test_first_user_registration_assigns_owner_role():
    ...
    from services_api.dashboard.v2.auth import RegisterIn, register
    ...
    with _pool_ctx(pool), patch("services_api.dashboard.v2._resend.send_email"):
        result = await register(body=body, request=_make_request())

    assert _insert_role_arg(conn) == "superadmin", "First user must be granted global superadmin role"
...
async def test_second_user_registration_gets_user_role():
    ...
    assert _insert_role_arg(conn) == "user", "Second user must get global role='user', not superadmin"
    assert result["status"] == "ok"
...
async def test_duplicate_email_raises_409_and_inserts_nothing():
    ...
    insert_user_calls = [c for c in conn.fetchval.await_args_list if c.args and "INSERT INTO users" in c.args[0]]
...
async def test_workspace_member_role_is_always_owner_for_own_workspace():
    ...
    assert _workspace_member_role_arg(conn) == "owner", (
        "workspace_members.role must be 'owner' for the workspace creator, regardless of global users.role"
    )
    assert result["status"] == "ok"
