...
ship: ## 🚀 Weekly deploy: act CI (exact GitHub runner) → merge develop → push main
	...
	@git checkout develop
	@echo ""
	@echo "Triggering all GitHub Actions workflows on main..."
	@gh workflow run build.yml           --repo saurabhrawat-gh/Autoniix --ref main 2>&1 && echo "✓  build.yml"       || echo "⚠  build.yml failed to queue"
	@gh workflow run ci.yml             --repo saurabhrawat-gh/Autoniix --ref main 2>&1 && echo "✓  ci.yml"          || echo "⚠  ci.yml failed to queue"
	@gh workflow run equivalence.yml    --repo saurabhrawat-gh/Autoniix --ref main 2>&1 && echo "✓  equivalence.yml" || echo "⚠  equivalence.yml failed to queue"
	@gh workflow run proto-validate.yml --repo saurabhrawat-gh/Autoniix --ref main 2>&1 && echo "✓  proto-validate"  || echo "⚠  proto-validate failed to queue"
	@gh workflow run rust-build.yml     --repo saurabhrawat-gh/Autoniix --ref main 2>&1 && echo "✓  rust-build.yml"  || echo "⚠  rust-build.yml failed to queue"
	@gh workflow run versions-in-sync.yml --repo saurabhrawat-gh/Autoniix --ref main 2>&1 && echo "✓  versions-in-sync" || echo "⚠  versions-in-sync failed to queue"
	@echo ""
	@echo "═══════════════════════════════════════════════════════"
	@echo "✅  Ship complete!"
	@echo "    All 6 workflows queued on main (workflow_dispatch)."
	@echo "    Zero runs will ever appear on develop. Ever."
