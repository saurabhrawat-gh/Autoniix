...
function step1_fixBeliefRegistry(ss) {
  ...
  const CORRECT_HEADERS = [
    'belief_id', 'channel_id', 'belief', 'counter_narrative', 'angle',
    'topic', 'belief_status', 'times_used', 'first_used_date',
    'last_used_date', 'cooling_until_date', 'video_id', 'idea_score'
  ];
  ...
  const existingHeaders = sheet.getRange(1, 1, 1, sheet.getMaxColumns()).getValues()[0]
    .filter(h => h !== '');
...
function step2_setupChannelDNA(ss) {
  ...
  const ALL_HEADERS = [
    'channel_id', 'channel_name', 'youtube_channel_id', 'niche', 'sub_niche',
    'belief_territory', 'intellectual_lens', 'topic_domain', 'brand_voice',
    'narrative_rhythm', 'emotional_contract', 'content_style', 'content_mode',
    'planned_duration', 'target_word_count', 'max_script_words_long',
    'min_script_words_long', 'max_script_words_short', 'min_script_words_short',
    'videos_per_week_long', 'videos_per_week_short', 'long_form_duration',
    'short_form_duration', 'primary_format_long', 'primary_format_short',
    'target_audience', 'cta_style_long', 'cta_style_short',
    'hook_length_seconds_long', 'hook_length_seconds_short',
    'retention_target_long', 'retention_target_short', 'forbidden_words',
    'thumbnail_style', 'primary_color', 'secondary_color', 'max_thumbnail_words',
    'posting_frequency', 'weekly_day', 'timezone', 'competitor_channels',
    'topics_queue', 'elevenlabs_voice_id', 'voice_stability', 'voice_similarity',
    'voice_style', 'video_template', 'status',
    // NEW columns (C-Optimized + visual identity)
    'words_per_video_long', 'words_per_video_short', 'visual_only_ratio',
    'font_family', 'caption_style', 'color_grade_preset', 'music_mood_default',
    'sfx_density', 'pacing_style', 'intro_template', 'outro_template',
    'target_ctr', 'target_avd_percent', 'max_daily_api_spend', 'human_review_required'
  ];
  ...
  var channels = [
    ...
    },
    {
      ...
      topic_domain: 'anxiety science, cortisol, fight-or-flight, vagus nerve, panic attacks, stress management, nervous system',
    ...
    // ──── FINANCE: Money Decoded ────
    {
      ...
      topic_domain: 'index funds, stocks, compound interest, ETFs, portfolio basics, risk management, dollar cost averaging',
    ...
    },
    {
      ...
      topic_domain: 'spending psychology, pricing tricks, financial biases, scarcity mindset, wealth mindset, consumer behavior',
    ...
    // ──── PSYCHOLOGY: Mind Shifts ────
    {
      ...
      topic_domain: 'manipulation tactics, cognitive biases, persuasion science, dark triad, social engineering, influence',
...
function step3_setupOutputLog(ss) {
  ...
  var NEW_COLUMNS = [
    'research_brief_url', 'script_v1_url', 'script_v2_url', 'script_v3_url',
    'direction_plan_url', 'storyboard_url', 'voiceover_full_url',
    'voiceover_scenes_urls', 'thumbnail_variants_urls', 'rendered_video_url',
    'score_report_url', 'youtube_metadata_url', 'checkpoint',
    'checkpoint_data_url', 'paused_at', 'resume_from', 'total_cost'
  ];
...
function step4_createTrendIntelligence(ss) {
  ...
  var headers = [
    'trend_id', 'channel_id', 'niche', 'trend_type', 'trend_title',
    'trend_description', 'source', 'source_url', 'detected_at',
    'relevance_score', 'virality_potential', 'competition_level',
    'freshness', 'status', 'used_in_video_id', 'expires_at'
  ];
...
function step5_createApiUsageTracker(ss) {
  ...
  var headers = [
    ...
    'dalle_calls', 'youtube_api_units', 'pixabay_calls', 'pexels_calls',
    'serpapi_calls', 'remotion_renders', 'total_cost', 'channel_id', 'content_id'
  ];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  Logger.log('  Created with ' + headers.length + ' columns');
}
...
function runStep1Only() { step1_fixBeliefRegistry(SpreadsheetApp.getActiveSpreadsheet()); }
function runStep2Only() { step2_setupChannelDNA(SpreadsheetApp.getActiveSpreadsheet()); }
function runStep3Only() { step3_setupOutputLog(SpreadsheetApp.getActiveSpreadsheet()); }
function runStep7Only() { step7_seedBeliefRegistry(SpreadsheetApp.getActiveSpreadsheet()); }
