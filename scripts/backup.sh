...
#   30 3 * * * /opt/youtube-automation/scripts/backup.sh >> /var/log/yt-backup.log 2>&1
#
...
docker run --rm --network yt-automation_yt-net \
    ...
    minio/mc:latest -- mirror --quiet --overwrite \
        "local/${S3_BUCKET:-yt-automation}/prod" /out/prod || true
docker run --rm --network yt-automation_yt-net \
    -v "$(pwd)/${OUT_DIR}/minio:/out" \
    -e MC_HOST_local="http://${S3_ACCESS_KEY:-minioadmin}:${S3_SECRET_KEY:-minioadmin}@minio:9000" \
    minio/mc:latest -- mirror --quiet --overwrite \
        "local/${S3_BUCKET:-yt-automation}/public" /out/public || true
