...
#   30 3 * * * /opt/autonix/scripts/backup.sh >> /var/log/autonix-backup.log 2>&1
#
...
docker run --rm --network autonix_autonix-net \
    ...
    minio/mc:latest -- mirror --quiet --overwrite \
        "local/${S3_BUCKET:-autonix}/prod" /out/prod || true
docker run --rm --network autonix_autonix-net \
    -v "$(pwd)/${OUT_DIR}/minio:/out" \
    -e MC_HOST_local="http://${S3_ACCESS_KEY:-minioadmin}:${S3_SECRET_KEY:-minioadmin}@minio:9000" \
    minio/mc:latest -- mirror --quiet --overwrite \
        "local/${S3_BUCKET:-autonix}/public" /out/public || true
