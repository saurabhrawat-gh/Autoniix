    ...
    >
      ...
      <motion.div variants={sectionVariants} className="grid grid-cols-2 lg:grid-cols-4 gap-3 !mt-4">
        {loading ? Array.from({ length: 4 }).map((_, i) => (
          ...
          <StatCard flash={wsFlash} label="Active Jobs" value={runningJobs.length}
          ...
          <div className="divide-y divide-border">
            <AnimatePresence initial={false}>
              {recentJobs.map(j => (
                ...
                >
                  <Link href={`/dashboard/jobs/${j.content_id}`}
...
function StatCard({
  label, value, sub, color, icon, href, progress, progressColor, flash, ecgState,
}: {
  label: string;
  value: string | number;
  sub?: string;
  ...
  const baseClass = cn('rounded-xl border border-border bg-surface-0 p-4 transition-colors relative overflow-hidden', flash && 'ws-flash');
  return (
    <motion.div
      ref={cardRef}
