...
pub fn routes(pool: PgPool) -> Router {
    Router::new()
        .route(
            ...
            get(get_finishing_config).put(put_finishing_config),
        )
...
async fn get_finishing_config(
    ...
    let exists = sqlx::query_scalar!("SELECT 1 FROM channels WHERE channel_id = $1", channel_id,)
        .fetch_optional(&pool)
        .await
...
async fn put_finishing_config(
    ...
    if let Some(ref p) = body.color_grade_preset {
        if !PRESET_KEYS.contains(&p.as_str()) {
            return Err(ApiError::Validation(format!(
                "color_grade_preset must be one of {PRESET_KEYS:?}"
            )));
        }
    }
    if let Some(v) = body.audio_loudness_lufs {
        if !(-24.0..=-9.0).contains(&v) {
            return Err(ApiError::Validation(
                "audio_loudness_lufs must be between -24 and -9".into(),
            ));
        }
    }
    if let Some(v) = body.audio_true_peak_dbtps {
        if !(-6.0..=-0.1).contains(&v) {
            return Err(ApiError::Validation(
                "audio_true_peak_dbtps must be between -6 and -0.1".into(),
            ));
        }
    }
...
async fn list_presets(AuthUser(_p): AuthUser) -> Json<Value> {
    Json(json!({
