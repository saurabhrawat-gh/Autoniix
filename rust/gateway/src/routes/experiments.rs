//! Thin proxy routes for `/api/v2/experiments/**` — forwards all traffic to
//! the Python BFF (`PYTHON_BFF_URL`).
...
pub fn routes() -> Router {
    Router::new()
        .route("/api/v2/experiments", any(handle))
        .route("/api/v2/experiments/*path", any(handle))
}
